...
interface DashboardTabProps {
  ...
  accounts: Account[];   // sigue llegando desde page.tsx via useAccounts
}
...
const ESTADO_CFG: Record<Estado, EstadoCfg> = {
  ...
  estimacion: { color: '#69F0AE',               glow: 'rgba(105,240,174,0.12)', badge: 'Estimación', badgeClass: 'text-[#69F0AE]/70 bg-[#00C853]/10 border-[#00C853]/20', heroLabel: 'Disponible estimado hoy' },
  bien:       { color: '#00E676',               glow: 'rgba(0,230,118,0.10)',   badge: 'Vas bien',   badgeClass: 'text-[#00E676] bg-[#00E676]/10 border-[#00E676]/25',    heroLabel: 'Podés gastar hoy' },
  cuidado:    { color: '#FFD740',               glow: 'rgba(255,215,64,0.10)',  badge: 'Cuidado',    badgeClass: 'text-[#FFD740] bg-[#FFD740]/10 border-[#FFD740]/20',    heroLabel: 'Podés gastar hoy' },
  mal:        { color: '#FF5252',               glow: 'rgba(255,82,82,0.10)',   badge: 'Estás justo',badgeClass: 'text-[#FF5252] bg-[#FF5252]/10 border-[#FF5252]/20',    heroLabel: 'Disponible' },
};
  ...
  return (
    <div className="rounded-2xl overflow-hidden" style={{ background: '#111714', border: '1px solid rgba(255,255,255,0.05)' }}>
      ...
      <div className="px-4 pb-4 grid grid-cols-2 gap-2">
        <div className="rounded-xl px-3 py-2.5" style={{ background: esteMes > 0 ? 'rgba(255,82,82,0.06)' : 'rgba(255,255,255,0.03)', border: esteMes > 0 ? '1px solid rgba(255,82,82,0.15)' : '1px solid rgba(255,255,255,0.05)' }}>
        ...
        <div className="rounded-xl px-3 py-2.5" style={{ background: proxMes > 0 ? 'rgba(255,215,64,0.05)' : 'rgba(255,255,255,0.03)', border: proxMes > 0 ? '1px solid rgba(255,215,64,0.12)' : '1px solid rgba(255,255,255,0.05)' }}>
...
export default function DashboardTab({
  ...
  // ── FUENTE ÚNICA DE DATOS: useSimpleSupabase ──────────────────────────────
  // Ya no usamos useSimpleAdaptedData — accedemos directamente a las
  // transacciones en su formato nativo (type, amount, category, transaction_date)
  const { transactions: allTransactions, goals, installments, createAccount, refresh } =
    useSimpleSupabase();
  ...
  // ── Transacciones filtradas por mes ───────────────────────────────────────
  // Accedemos directamente a los campos nativos: type, amount, transaction_date, category
  const txMes = useMemo(
  ...
  const financials = useMemo(() => {
    // Ingresos y gastos usando campos nativos (sin adaptador)
    ...
    // Cuentas (vienen de useAccounts via prop)
    ...
    // Ingreso de referencia: real si está registrado, estimado del onboarding si no
    ...
    // Ritmo: promedio diario actual vs ideal
  ...
  const estado = useMemo((): Estado => {
    ...
    if (txMes.length < 3 && totalIngresos > 0) return 'estimacion';
    ...
    return 'mal';
  }, [totalIngresos, totalGastos, accounts.length, txMes.length, ingresoRef, realDisponible, neto]);
  ...
  const alertas = useMemo(() => {
    ...
    if (cuotasEsteMes > 0 && heroAmount < cuotasEsteMes) {
      items.push({ icon: <CreditCard size={13} />, text: `Tenés ${fmt(cuotasEsteMes)} en cuotas este mes y no alcanza el saldo`, color: '#FF5252', onClick: onNavigateToChat });
    }
    if (ratioRitmo > 1.3 && ingresoRef > 0) {
      items.push({ icon: <Zap size={13} />, text: `Gastás ${Math.round((ratioRitmo - 1) * 100)}% más rápido que lo ideal para llegar a fin de mes`, color: '#FFD740', onClick: onNavigateToChat });
    }
  ...
  return (
    <div className="space-y-3 pb-24 md:pb-6" style={{ opacity: mounted ? 1 : 0, transition: 'opacity 0.3s ease' }}>

      {/* ── HERO ── */}
      <div className="rounded-2xl p-5 relative overflow-hidden" style={{ background: '#0D1410', border: '1px solid rgba(255,255,255,0.06)', boxShadow: cfg.glow !== 'transparent' ? `0 0 40px ${cfg.glow}` : 'none' }}>
        <div className="flex items-center justify-between mb-4">
          <p className="text-white/30 text-xs">
            {onboarding.nombre ? `Hola, ${onboarding.nombre}` : 'Tu resumen'}
            {' · '}{MESES[hoy.getMonth()]}
          </p>
          <span className={`text-[10px] font-semibold px-2.5 py-1 rounded-full border ${cfg.badgeClass}`}>{cfg.badge}</span>
        </div>
        ...
        {!showHeroNumber ? (
          ...
          <div className="mb-4">
            ...
            <p className="text-[52px] font-bold tracking-tight tabular-nums leading-none" style={{ color: cfg.color }}>
              {heroAmount < 0 ? '-' : ''}{fmtCompact(heroAmount)}
            </p>
            {onboarding.objetivo_ahorro > 0 && (
              <p className="text-white/25 text-xs mt-2">Ahorrando {fmt(onboarding.objetivo_ahorro)}/mes</p>
            )}
          </div>
        )}

        {showHeroNumber && (
          <div className="rounded-xl px-3 py-2.5 mb-4" style={{ background: vaALlegar ? 'rgba(0,230,118,0.06)' : 'rgba(255,82,82,0.06)', border: vaALlegar ? '1px solid rgba(0,230,118,0.12)' : '1px solid rgba(255,82,82,0.12)' }}>
            ...
            {topCat && (
              <p className="text-[10px] text-white/25 mt-1 ml-[20px]">Mayor gasto: {topCat[0]} ({fmtCompact(topCat[1])})</p>
            )}
          </div>
        )}
        ...
        <div className="mb-4">
          ...
          <div className="h-px bg-white/6 rounded-full overflow-hidden">
            <div className="h-full rounded-full transition-all duration-700" style={{ width: `${pctMes}%`, backgroundColor: cfg.color, opacity: 0.5 }} />
          </div>
        </div>

        <div className="flex gap-2">
          <button onClick={onNavigateToChat} className="flex-1 flex items-center justify-center gap-1.5 py-2.5 rounded-xl bg-white/5 border border-white/8 hover:bg-white/8 active:scale-[.97] transition-all text-white/55 text-xs font-medium">
            ✏️ Registrar gasto
          </button>
          {gastoDiario > 0 && (
            <div className="flex-1 flex items-center justify-center py-2.5 rounded-xl border text-xs font-medium" style={{ background: `${cfg.color}0D`, borderColor: `${cfg.color}20`, color: `${cfg.color}99` }}>
              {fmt(gastoDiario)}/día
            </div>
          )}
        </div>
      ...
      <div className="grid grid-cols-3 gap-2">
        {([
          ...
          <div key={item.label} className="rounded-xl p-3 text-center" style={{ background: '#111714', border: '1px solid rgba(255,255,255,0.05)' }}>
      ...
      {accounts.length > 0 && (
        <div className="rounded-2xl overflow-hidden" style={{ background: '#111714', border: '1px solid rgba(255,255,255,0.05)' }}>
          <div className="flex items-center justify-between px-4 pt-4 pb-3">
            ...
            <button onClick={() => setShowAccountModal(true)} className="text-[#00C853]/50 text-xs hover:text-[#00C853] transition-colors flex items-center gap-1">
          ...
          {liquidAccounts.length > 0 && (
            <div className="flex gap-2 px-4 overflow-x-auto pb-3 scrollbar-hide">
              {liquidAccounts.map(acc => (
                <div key={acc.id} className="flex-shrink-0 rounded-xl px-3 py-2.5 flex items-center gap-2 min-w-[130px]" style={{ background: acc.color ? `${acc.color}10` : 'rgba(255,255,255,0.04)', border: `1px solid ${acc.color ? `${acc.color}20` : 'rgba(255,255,255,0.06)'}` }}>
          ...
          {creditAccounts.length > 0 && totalCredit > 0 && (
            <div className="mx-4 pt-3 border-t border-white/5">
              <p className="text-[#FF5252]/50 text-xs mb-2">💳 Comprometido en tarjetas: {fmt(totalCredit)}</p>
            </div>
          )}

          <div className="mx-4 mb-4 mt-1 rounded-xl px-3 py-2.5" style={{ background: 'rgba(255,255,255,0.03)', border: '1px solid rgba(255,255,255,0.06)' }}>
      ...
      {metasActivas.length > 0 && (
        <div className="rounded-2xl overflow-hidden" style={{ background: '#111714', border: '1px solid rgba(255,255,255,0.05)' }}>
          <div className="flex items-center justify-between px-4 pt-4 pb-2">
            ...
            <button onClick={onNavigateToMetas} className="flex items-center gap-1 text-[#00C853]/50 text-xs hover:text-[#00C853] transition-colors">
      ...
      <div className="rounded-2xl overflow-hidden" style={{ background: '#111714', border: '1px solid rgba(255,255,255,0.05)' }}>
        <div className="flex items-center justify-between px-4 pt-4 pb-2">
          ...
          {recientes.length > 0 && (
            <button onClick={onNavigateToChat} className="flex items-center gap-1 text-[#00C853]/50 text-xs hover:text-[#00C853] transition-colors">
        ...
        {recientes.length === 0 ? (
          <button onClick={onNavigateToChat} className="w-full flex items-center gap-3 px-4 pb-4 text-left group">
          ...
          <div className="pb-2">
            {recientes.map((t, idx) => (
              <div key={t.id} className={`flex items-center gap-3 px-4 py-2.5 ${idx < recientes.length - 1 ? 'border-b border-white/[0.04]' : ''}`}>
                ...
                <p className="text-sm font-medium tabular-nums shrink-0" style={{ color: t.type === 'ingreso' ? '#69F0AE' : 'rgba(255,255,255,0.4)' }}>
      ...
      {allTransactions.length === 0 && accounts.length === 0 && (
        <div className="rounded-2xl p-5" style={{ background: '#111714', border: '1px solid rgba(255,255,255,0.05)' }}>
          ...
          <div className="space-y-1">
            {([
              ...
              <button key={idx} onClick={item.action} className="w-full flex items-center gap-3 text-left p-2.5 rounded-xl hover:bg-white/4 active:bg-white/6 transition-colors group">
