// ============================================================
// AI Wallet — Motor de IA compartido
// lib/ai-engine.ts
//
// Lógica extraída de app/api/chat/route.ts sin modificar nada.
// Usada por:
...
export interface RequestContext {
  ...
  canal?: Canal
  budgets?: Array<{ categoria: string; limite: number; gastado: number; estado: string }>
  goals?: Array<{ nombre: string; objetivo: number; actual: number; faltante: number }>
  alertas?: string[]
  ...
  historico?: {
    ...
    categorias: Array<{ categoria: string; tipo: string; promedio_mensual: number; gasto_este_mes: number }>
  }
  historico_completo?: string
  simulaciones?: Array<{ categoria: string; gastoMensualActual: number; ahorroMensual: number; ahorro6Meses: number; ahorro12Meses: number }>
  ultimas_transacciones?: Array<{ descripcion: string; categoria: string; monto: number; tipo: string; fecha: string }>
  gasto_por_semana_promedio?: number
  comparativa_semana?: { promedioLunesViernes: number; promedioSabadoDomingo: number; factorFinDeSemana: number }
  ...
  cumplimiento_ahorro?: Array<{ mes: string; realAhorrado: number; cumplido: boolean; pct: number }>
  cuentas?: Array<{ id: string; nombre: string; tipo: string; saldo: number; icono?: string }>
  resumen_cuentas?: { total_liquid: number; total_comprometido: number; real_disponible: number; cuotas_este_mes: number }
...
// ─────────────────────────────────────────────────────────────────────────────
// SYSTEM PROMPTS v3.0 — copia exacta del route.ts original
// ─────────────────────────────────────────────────────────────────────────────
...
const SYSTEM_PROMPT_WHATSAPP_ADDON = `

━━━ CANAL: WHATSAPP — REGLAS ADICIONALES ━━━━━━━━━━━━━━━━━━━

Estás respondiendo por WhatsApp. Ajustá el formato:
- Sin asteriscos, sin markdown de ningún tipo en mensaje_respuesta.
- Máximo 3 oraciones en mensaje_respuesta. En WhatsApp menos es más.
- Los números siempre con formato: "$12.500" no "$12500".`
...
// ─────────────────────────────────────────────────────────────────────────────
// INTENT CLASSIFIER v3.0 — copia exacta del route.ts original
// ─────────────────────────────────────────────────────────────────────────────

export function classifyIntent(message: string): BackendIntent {
  ...
  const patronesHistoricos = /\b(este (año|anio)|en el año|historico|acumulado|desde (enero|el año)|cuanto (llevo|gaste en|gastaste)|mes mas caro|mes (más|mas) caro|mis ultimos|últimos (gastos|movimientos|5|10)|ultimas (transacciones|operaciones)|semana vs fin|fin de semana|mas los (sabados|fines)|dias sin|hace cuanto|cuando fue la ultima|cuanto llevo (sin|gastando)|vengo cumpliendo|meses (cumpli|ahorre|ahorré)|por semana (en promedio|cuanto)|promedio (semanal|por semana)|en total (este|el) (año|anio)|cuanto (subi|baje|cambio) (mis|los|en)|comparado con el (mes|anterior|anio)|gaste mas|gaste menos|subi o baje)\b/.test(msg)
...
export function buildSystemPrompt(intent: BackendIntent, canal: Canal = 'app'): string {
  const whatsappAddon = canal === 'whatsapp' ? SYSTEM_PROMPT_WHATSAPP_ADDON : ''
  switch (intent) {
    case 'registro':           return SYSTEM_PROMPT_BASE + whatsappAddon + SYSTEM_PROMPT_REGISTRO
    case 'consulta_simple':    return SYSTEM_PROMPT_BASE + whatsappAddon + SYSTEM_PROMPT_CONSULTA
    case 'consulta_historica': return SYSTEM_PROMPT_BASE + whatsappAddon + SYSTEM_PROMPT_HISTORICO
    case 'simulacion':         return SYSTEM_PROMPT_BASE + whatsappAddon + SYSTEM_PROMPT_SIMULACION
    case 'planificacion':      return SYSTEM_PROMPT_BASE + whatsappAddon + SYSTEM_PROMPT_PLANIFICACION
    case 'gestion_cuentas':    return SYSTEM_PROMPT_BASE + whatsappAddon + SYSTEM_PROMPT_GESTION_CUENTAS
    case 'complejo':           return SYSTEM_PROMPT_BASE + whatsappAddon + SYSTEM_PROMPT_COMPLEJO + SYSTEM_PROMPT_HISTORICO + SYSTEM_PROMPT_SIMULACION
  }
}

// ─────────────────────────────────────────────────────────────────────────────
// buildDynamicContext — copia exacta del route.ts original
// ─────────────────────────────────────────────────────────────────────────────
...
export function buildDynamicContext(
  ...
  if (intent === 'registro') {
    ...
    return [
      ...
      `CATEGORÍAS DISPONIBLES (con estado actual del mes):`, categorias, ``,
      cuentaResuelta,
  ...
  if (intent === 'simulacion') {
    ...
    const metasStr = context.goals
      ...
      .join('\n') ?? 'Sin metas activas'
    return [
      `FECHA: ${fecha}`, `USUARIO: ${usuario}`, estadoBase, ``,
      `SIMULACIONES PRE-CALCULADAS (50% de reducción por categoría discrecional):`,
      simulacionesStr, ``, `METAS ACTIVAS:`, metasStr, ``,
      ...
      ``, `CONTEXTO HISTÓRICO:`, context.historico_completo ?? 'Sin historial disponible',
    ].join('\n')
  }
  ...
  if (intent === 'consulta_historica') {
    return [
      `FECHA: ${fecha}`, `USUARIO: ${usuario}`, estadoBase, ``,
      `CONTEXTO HISTÓRICO COMPLETO:`, context.historico_completo ?? 'Sin historial disponible', ``,
      `ESTADO ACTUAL MES:`, context.resumen_financiero ?? 'Sin datos del mes actual', ``,
  ...
  if (intent === 'planificacion') {
    ...
    return [
      ...
      `METAS ACTIVAS:`, metasStr, ``,
      `CONTEXTO HISTÓRICO:`, context.historico_completo ?? 'Sin historial', ``,
      `SIMULACIONES DISPONIBLES:`,
      context.simulaciones?.map(s => `- ${s.categoria}: ahorra $${s.ahorroMensual.toLocaleString('es-AR')}/mes si recorta 50%`).join('\n') ?? 'Sin simulaciones',
      ``, `HISTORIAL DETALLADO:`,
  ...
  if (intent === 'gestion_cuentas') {
    ...
    return [
      `FECHA: ${fecha}`, `USUARIO: ${usuario}`, ``,
      `CUENTAS ACTUALES:`, listaCuentas, ``, `RESUMEN:`,
  ...
  if (intent === 'consulta_simple') {
    ...
    return [
      `FECHA: ${fecha}`, `USUARIO: ${usuario}`, ``,
      estadoBase, alertasStr, ``,
      `PRESUPUESTOS:`, presupuestos, ``,
      `RESUMEN FINANCIERO:`, context.resumen_financiero ?? 'Sin datos disponibles',
  ...
  return [
    `FECHA: ${fecha}`, `USUARIO: ${usuario}`, ``,
    `SITUACIÓN FINANCIERA ACTUAL:`, context.resumen_financiero ?? 'Sin datos disponibles', ``,
    estadoBase, ``,
    `CATEGORÍAS EXACTAS:`,
    context.budgets?.map(b => `- "${b.categoria}": $${b.gastado.toLocaleString('es-AR')}/$${b.limite.toLocaleString('es-AR')} (${b.estado})`).join('\n') ?? 'Sin categorías',
    ``, `METAS:`,
    context.goals?.map(g => `- ${g.nombre}: $${g.actual.toLocaleString('es-AR')} de $${g.objetivo.toLocaleString('es-AR')} (falta $${g.faltante.toLocaleString('es-AR')})`).join('\n') ?? 'Sin metas',
    ``, `CUENTAS:`, listaCuentasCompleta, ``,
    `CUENTA RESUELTA: ${serverResolvedAccountId ? `id ${serverResolvedAccountId}` : 'ninguna — account_id = null'}`,
    ``, context.perfil_coach ?? '', ``,
    `ALERTAS:`, context.alertas?.map(a => `- ${a}`).join('\n') ?? 'Sin alertas', ``,
    `CONTEXTO HISTÓRICO:`, context.historico_completo ?? 'Sin historial', ``,
    `HISTORIAL DETALLADO:`,
    `Gasto promedio mensual: $${context.historico?.gasto_mensual_promedio?.toLocaleString('es-AR') ?? 'sin datos'}`,
    `Gasto mínimo mensual: $${context.historico?.gasto_minimo_mensual?.toLocaleString('es-AR') ?? 'sin datos'}`,
    ``, `CATEGORÍAS ANALIZADAS:`,
    context.historico?.categorias?.map(c => `- ${c.categoria} [${c.tipo.toUpperCase()}]: prom $${c.promedio_mensual?.toLocaleString('es-AR')}/mes | este mes: $${c.gasto_este_mes?.toLocaleString('es-AR') ?? '0'}`).join('\n') ?? 'Sin historial',
    ``, `RESUMEN CUENTAS:`,
...
// ─────────────────────────────────────────────────────────────────────────────
// Historia y max_tokens — copia exacta del route.ts original
// ─────────────────────────────────────────────────────────────────────────────
...
export function getMaxTokens(intent: BackendIntent, canal: Canal = 'app'): number {
  const limits: Record<BackendIntent, number> = {
    registro: 450, gestion_cuentas: 250, consulta_simple: 400,
    consulta_historica: 600, simulacion: 700, planificacion: 900, complejo: 800,
  }
  if (canal === 'whatsapp') return Math.min(limits[intent], 350)
...
// ─────────────────────────────────────────────────────────────────────────────
// cleanAndParseAIResponse — copia exacta del route.ts original
// ─────────────────────────────────────────────────────────────────────────────

export function cleanAndParseAIResponse(raw: string): ChatResponse {
...
// ─────────────────────────────────────────────────────────────────────────────
// resolveAccount — copia exacta del route.ts original
// ─────────────────────────────────────────────────────────────────────────────
...
// ─────────────────────────────────────────────────────────────────────────────
// Helpers internos — copia exacta del route.ts original
// ─────────────────────────────────────────────────────────────────────────────

interface TransactionPayload {
  description?: string; descripcion?: string
  amount?: number; monto?: number
  category?: string; categoria?: string
  type?: string; tipo?: string
  transaction_date?: string; fecha?: string
  confirmed?: boolean
...
async function saveTransactionsToSupabase(
  ...
  const transactionsToInsert: TransactionInsert[] = transacciones.map(tx => {
    ...
    const goalMatch          = normalizedCategory === 'ahorro'
  ...
  const { data, error } = await supabaseClient
    .from('transactions')
    .insert(transactionsToInsert.map(({ installment_count: _ic, first_due_month: _fd, ...rest }) => rest))
    .select()
  ...
  if (data && data.length > 0) {
    for (let idx = 0; idx < data.length; idx++) {
      ...
      const { data: accData } = await supabaseClient
        .from('accounts').select('type')
        .eq('id', saved.account_id).eq('user_id', userId).single()
      ...
      const firstDueMonth = (txExtra as TransactionInsert & { first_due_month?: string }).first_due_month ?? new Date().toISOString().slice(0, 7)

      await generateInstallments(saved.id, saved.account_id, userId, saved.amount, installCount, firstDueMonth, supabaseClient)
    }
  }
}
  ...
  switch (action) {
    ...
    case 'CREATE_GOAL': {
      const { error } = await supabaseClient.from('goals').insert({
        name:           ensureGoalEmoji(String(data?.name ?? data?.title ?? 'Meta sin nombre')),
        target_amount:  data?.target_amount,
        current_amount: data?.current_amount ?? 0,
        target_date:    data?.target_date ?? null,
        description:    data?.description ?? '',
        icon:           data?.icon ?? '🎯',
        color:          data?.color ?? 'text-emerald-500',
        user_id:        userId,
      })
      if (error) throw handleSupabaseError(error)
    ...
    case 'CREATE_BUDGET': {
      const { error } = await supabaseClient.from('budgets').insert({
        ...
        month_period: String(data?.month_period ?? '') || new Date().toISOString().slice(0, 7),
        user_id:      userId,
      })
      if (error) throw handleSupabaseError(error)
    ...
    case 'CREATE_ACCOUNT': {
      const accData = data as { name: string; type: 'liquid' | 'credit' | 'savings'; balance: number; credit_limit?: number; closing_day?: number; due_day?: number; icon?: string; color?: string; set_as_default?: boolean }
      if (accData.set_as_default) {
        await supabaseClient.from('accounts').update({ is_default: false })
          .eq('user_id', userId).eq('type', accData.type).eq('is_default', true)
      }
      const { data: account, error } = await supabaseClient.from('accounts').insert({
        user_id: userId, name: accData.name, type: accData.type?.toLowerCase(),
        balance: accData.balance ?? 0, credit_limit: accData.credit_limit ?? null,
        closing_day: accData.closing_day ?? null, due_day: accData.due_day ?? null,
        icon: accData.icon ?? null, color: accData.color ?? null,
        is_default: accData.set_as_default ?? false, is_active: true, currency: 'ARS',
      }).select().single()
      if (error) throw error
      return { success: true, mensaje_respuesta: 'Cuenta creada exitosamente', action: 'CREATE_ACCOUNT', data: account as Record<string, unknown> }
    }
    case 'UPDATE_ACCOUNT_BALANCE': {
      const balData = data as { account_name: string; new_balance: number }
      const { data: accounts, error } = await supabaseClient
        .from('accounts').select('id, name, type, balance')
        .eq('user_id', userId).eq('is_active', true)
        .ilike('name', `%${balData.account_name}%`)
      if (error) throw handleSupabaseError(error)
      if (!accounts || accounts.length === 0) {
        return { success: false, suggestion: `No encontré ninguna cuenta con ese nombre. ¿Querés crearla?`, message: `No encontré ninguna cuenta con ese nombre.` }
      }
      const target = accounts[0] as { id: string; name: string }
      const { error: updateErr } = await supabaseClient.from('accounts')
        .update({ balance: balData.new_balance }).eq('id', target.id).eq('user_id', userId)
      if (updateErr) throw handleSupabaseError(updateErr)
      return { success: true, message: `Balance actualizado en ${target.name}` }
    }
    case 'UPDATE_GOAL_PROGRESS': {
      const goalName        = String(data?.goal_name ?? '')
      const amount          = Number(data?.amount ?? 0)
      const createIfMissing = Boolean(data?.create_if_missing ?? true)
      ...
      const { error } = await supabaseClient.from('goals')
        ...
        .eq('id', targetGoal.id).eq('user_id', userId)
      if (error) throw handleSupabaseError(error)
      return { success: true, message: 'Progreso actualizado' }
    }
    case 'QUERY_BUDGET':
...
// ─────────────────────────────────────────────────────────────────────────────
// runAIEngine — punto de entrada principal
// ─────────────────────────────────────────────────────────────────────────────

export async function runAIEngine(input: EngineInput): Promise<EngineOutput> {
  const {
    message, context, history, userId,
    budgetsData, goalsData, accountsData,
    serverResolvedAccountId, unpaidInstallmentsTotal,
    canal = 'app', supabaseClient,
  } = input
  ...
  if (!process.env.GROQ_API_KEY) throw new Error('Groq API key no configurada')
  ...
  const systemPrompt   = buildSystemPrompt(intent, canal)
  const historySlice   = getHistorySlice(intent, history)
  const maxTokens      = getMaxTokens(intent, canal)

  if (process.env.NODE_ENV === 'development') {
    console.log('📊 ENGINE:', {
      intent, canal, maxTokens,
      estimated_input: estimateTokens(systemPrompt) + estimateTokens(dynamicContext) + estimateTokens(message),
    })
  }
  ...
  const aiResponse = cleanAndParseAIResponse(rawContent)
  ...
  return { aiResponse, serverResolvedAccountId }
}